...
/**
 * This is the model class for table "point_text_value_tb".
 *
 * The followings are the available columns in table 'point_text_value_tb':
 * @property string $id
...
class PointValueTb extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('point_id, datetime_record, point_text_value, user_id, remark', 'required'),
			array('point_id', 'length', 'max'=>9),
			array('point_text_value, remark', 'length', 'max'=>255),
			array('user_id', 'length', 'max'=>8),
			array('status_flag', 'length', 'max'=>1),
			array('created_date, modified_date', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, point_id, datetime_record, point_text_value, user_id, status_flag, remark, created_date, modified_date,category_id,section_id', 'safe', 'on'=>'search'),
		);
	}
